...
<%
	...
	try{
		...
		ResultSet rs2 = st.executeQuery("select *from users inner join carts where carts.email='"+email+"' and carts.status='bill'");
		while(rs2.next()){
...
<h3>Product Details</h3>
  <tr>
   ...
   <% ResultSet rs1 = st.executeQuery("select *from carts inner join product where carts.product_id=product.id and carts.email='"+email+"' and carts.status='bill'");
   while(rs1.next()){
   sno=sno+1;
   %>
  <tr>
